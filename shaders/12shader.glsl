...
vec3 beamSplit(inout Path path,inout Path reflPath, inout Path transPath){
    ...
    //set sky color-er
    origtv=reflPath.dat.reflectedRay;
    ...
    transmitDir=transPath.dat.refractedRay;
        ...
        //set sky color-er
    origtv=transPath.dat.refractedRay;
...
//    //====follow only the brightest initial ray
    while(path.keepGoing&&numIterate<2){    totalColor+=beamSplit(path,reflectedRay,transmittedRay);       numIterate+=1;
  }
////    
//    
